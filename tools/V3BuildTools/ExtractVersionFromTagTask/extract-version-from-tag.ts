...
async function run() {
    try {
        ...
        let buildNumber = (27_000 + parseInt(numberOfCommits[0])).toString() /* 30_0000 is the number that we used to deploy old version before, fo we have to continue with the version upper then 30000 */
...
function changeIOSVersion(buildVersion: string, versionName: string) {
...
function changeAndroidVersion(buildVersion: string, versionName: string) {
